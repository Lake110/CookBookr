...
class RecipeListView(ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["current_max_prep_time"] = self.request.GET.get(
            "max_prep_time", ""
        )
        context["current_max_cook_time"] = self.request.GET.get(
            "max_cook_time", ""
        )
...
def add_recipe(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                request,
                "Recipe and image submitted successfully!"
            )
...
def comment_edit(request, recipe_id, comment_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Don't reset approval status for edits -
...
def recipe_delete(request, recipe_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f'Recipe "{recipe_title}" has been deleted successfully!'
        )
...
def recipes_by_category(request, category_slug):
    ...
    recipes = Recipe.objects.filter(category=category_key)
    recipes = recipes.order_by("-created_at")
